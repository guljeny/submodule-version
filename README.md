...
- `sv.getResolution()` returns the selected version and selected-version dependencies for every resolved module. Before the first `init()` call it throws `NOT_INITIALIZED`; after an `init()` attempt it remains available even when initialization reports an error. A PubGrub error retains its best-effort partial resolution, while Git/npm failures retain the complete calculated result. A module without any version tag cannot be represented as a resolved entry.
...
- `sv.dangerousPut(url, version?, parentName?)` and `sv.dangerousDelete(name, parentName?)` apply their Git and `package.json` mutations without using PubGrub as a compatibility gate. Afterwards they perform a non-blocking resolve and store its complete or best-effort partial result before running `npm install`. `dangerousPut` checks out the newest local Git tag matching the requested range; an untagged module remains on its cloned HEAD.
