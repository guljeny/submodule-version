...
- `sv.getResolution()` returns the selected version and selected-version dependencies for every resolved module. Before the first `init()` call it throws `NOT_INITIALIZED`; after an `init()` attempt it remains available even when initialization reports an error. If PubGrub itself fails before producing a result, the stored resolution is empty; Git/npm failures retain the complete calculated result.
