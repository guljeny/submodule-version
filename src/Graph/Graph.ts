...
export class Graph {
  constructor (private pkgJSONManager: PkgJSONManager) {}

  public async build (): Promise<Record<string, IGraphEntry>> {
    const baseJson = await this.pkgJSONManager.read();
      ...
      await Promise.all(
        Object.entries(dependencies).map(async ([url, version]) => {
          ...
          const mJson = existEntry
            ? null
            : await this.pkgJSONManager.read(name);
